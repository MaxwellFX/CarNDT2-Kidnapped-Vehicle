...
void ParticleFilter::init(double x, double y, double theta, double std[]) {
	/* TODO:
     * Set the number of particles. Initialize all particles to first position (based on
...
void ParticleFilter::prediction(double delta_t, double std_pos[], double velocity, double yaw_rate) {
	 ...
	 * 
     * NOTE: 
     * When adding noise you may find std::normal_distribution and 
...
void ParticleFilter::updateWeights(double sensor_range, double std_landmark[], 
     ...
     * You will need to transform between the two systems.Keep in mind that this 
    ...
    for (int i = 0; i < num_particles; i++) {
		...
		// Create a vector to hold the map landmark that are within the sensor's range
        vector<LandmarkObs> vLandMarksInRange;
        ...
        for (unsigned int j = 0; j < map_landmarks.landmark_list.size(); j++) {
            ...
            if (fabs(x - landmarkX) <= sensor_range && fabs(y - landmarkY) <= sensor_range) {
                vLandMarksInRange.push_back(LandmarkObs{id, landmarkX, landmarkY});
            }
        }

        // Create a vecctor of observations that are transformed from vehicle coordinates to map ciirdubates
        ...
        for (unsigned int j = 0; j < observations.size(); j++) {
            ...
            mappedObservations.push_back(LandmarkObs{observations[j].id, x_trans, y_trans});
        }

        // Observation association to current landmark
        dataAssociation(vLandMarksInRange, mappedObservations);
        ...
        for (unsigned int j = 0; j < mappedObservations.size(); j++) {
            ...
            // Recrod the predicted landmark coordinates associated with current observations
            double landmarkX, landmarkY;
            for(unsigned int k = 0; k < vLandMarksInRange.size(); k++) {
                if (vLandMarksInRange[k].id == landmarkId) {
                    landmarkX = vLandMarksInRange[k].x;
                    landmarkY = vLandMarksInRange[k].y;
                    break;
                }
            }
...
void ParticleFilter::resample() {
    ...
    // Generate random distribution [0.0, max_weight]
